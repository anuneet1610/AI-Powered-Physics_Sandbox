import pygame
import random
import sandbox_objects.coords as coords
import sandbox_objects.config as config
import sandbox_objects.bodies as bodies
import sandbox_objects.wall as wall
import sandbox_objects.spring as spring
import sandbox_objects.collisions as collisions
import sandbox_objects.gravity_well as gravity_well
import sandbox_objects.state as state
from agents.edit_planner import edit_planner
from agents.edit_executor import apply_scene_edits
import math
...
def _next_id(obj_type):
    _id_counters[obj_type] = _id_counters.get(obj_type, 0) + 1
    return f"{obj_type}_{_id_counters[obj_type]}"

def assign_identity(obj, obj_type, role_label=None):
    """Attach id + role_label to any object, regardless of creation path."""
    if not hasattr(obj, "id") or obj.id is None:
        obj.id = _next_id(obj_type)
    if not hasattr(obj, "role_label") or obj.role_label is None:
        obj.role_label = role_label or obj.id
    return obj

def compute_pos_labels(objs, axis="x"):
    """Recomputed fresh every pause — never stored persistently."""
    if not objs:
        return {}
    key = (lambda o: o.x) if axis == "x" else (lambda o: o.y)
    ordered = sorted(objs, key=key)
    n = len(ordered)
    labels = {}
    for i, obj in enumerate(ordered):
        if i == 0:
            labels[obj.id] = "leftmost" if axis == "x" else "bottommost"
        elif i == n - 1:
            labels[obj.id] = "rightmost" if axis == "x" else "topmost"
        elif n % 2 == 1 and i == n // 2:
            labels[obj.id] = "middle"
        else:
            labels[obj.id] = f"{i+1} from {'left' if axis=='x' else 'bottom'}"
    return labels

def describe_scene(balls, rects, walls, springs):
    lines = []
    for b in balls:
        lines.append(
            f"Ball(id={b.id}, role={b.role_label}, pos={b.pos_label}): "
            f"x={b.x}, y={b.y}, vx={b.vx}, vy={b.vy}, radius={b.radius}"
        )
    for r in rects:
        lines.append(
            f"Rect(id={r.id}, role={r.role_label}, pos={r.pos_label}): "
            f"x={r.x}, y={r.y}, vx={r.vx}, vy={r.vy}, width={r.length}, height={r.width}"
        )
    for w in walls:
        lines.append(
            f"Wall(id={w.id}, role={w.role_label}, pos={w.pos_label}): "
            f"x1={w.x1}, y1={w.y1}, x2={w.x2}, y2={w.y2}"
        )
    for sp in springs:
        lines.append(
            f"Spring(id={sp.id}, role={sp.role_label}, pos={sp.pos_label}): "
            f"connects {sp.a.id} <-> {sp.b.id}"
        )
    return "\n".join(lines)

def run_pygame(final_state):
    balls = final_state["ball_obj"]
    rectangles = final_state["rect_obj"]
    springs = final_state["spring_obj"]
    walls = final_state["wall_obj"]
    wells = []
    NUM_BOUNDARY_WALLS = 0

    # Ensure every pre-existing object has an id/role_label,
    # whether or not the planner already set one.
    for b in balls:
        assign_identity(b, "ball", getattr(b, "role_label", None))
    for r in rectangles:
        assign_identity(r, "rect", getattr(r, "role_label", None))
    for w in walls:
        assign_identity(w, "wall", getattr(w, "role_label", None))
    for sp in springs:
        assign_identity(sp, "spring", getattr(sp, "role_label", None))

    def handle_pause_popup_submit(text):
        if not text.strip():
            return
        scene_description = describe_scene(balls, rectangles, walls, springs)
        edit_state = {"message": text, "scene_description": scene_description, "edit_plan": None}
        edit_state = edit_planner(edit_state)  # LangGraph edit call
        apply_scene_edits(edit_state["edit_plan"], balls, rectangles, walls, springs)

    pygame.init()

    import sandbox_objects.ui as ui
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if ui.handle_pause_popup_keydown(event, handle_pause_popup_submit):
                    continue
                ...
                if event.key == pygame.K_g:
                    ...
                    if state.paused:
                        state.pos_labels_x = compute_pos_labels(balls + rectangles, axis="x")
                        state.pos_labels_y = compute_pos_labels(balls + rectangles, axis="y")
                        ui.pause_popup_open()
                    else:
                        ui.pause_popup_close()

            elif event.type == pygame.MOUSEBUTTONDOWN:
                mx, my = pygame.mouse.get_pos()

                if state.paused and getattr(state, "pause_popup_active", False):
                    ui.pause_popup_handle_click(mx, my)
                ...
                # Panel click — toggle controlled ball / wire springs (always available)
                    ...
                    if state.mode == "SPRING":
                        ...
                        if state.spring_selected_obj is None:
                            ...
                            if existing:
                                ...
                                new_spring = spring.Spring(ball_a=state.spring_selected_obj, ball_b=clicked, id=_next_id("spring"))
                    ...
                    if state.mode == "WALL" and inside:
                        state.wall_drawing = True
                        state.wall_start_wx = wx
                        state.wall_start_wy = wy

                    elif state.mode == "WELL" and inside:
                        well = gravity_well.Gravity_Well(x=wx, y=wy, mass=1000, id = _next_id("well"))
                        wells.append(well)
                ...
                if state.wall_drawing:
                    ...
                    if math.sqrt(ddx * ddx + ddy * ddy) > 0.1:
                        new_wall = wall.Wall(x1=state.wall_start_wx, y1=state.wall_start_wy, x2=wx, y2=wy, id=_next_id("wall"))
...
if __name__ == "__main__":
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                    ...
                    if state.mode == "SPRING":
                        ...
                        if state.spring_selected_obj is None:
                            ...
                            if existing:
                                ...
                                new_spring = spring.Spring(ball_a=state.spring_selected_obj, ball_b=clicked,
                                                           id=_next_id("spring"))
                                springs.append(new_spring)
                            state.spring_selected_obj = None
                ...
                if state.wall_drawing:
                    ...
                    ddy = wy - state.wall_start_wy
                    if math.sqrt(ddx * ddx + ddy * ddy) > 0.1:
                        new_wall = wall.Wall(x1=state.wall_start_wx, y1=state.wall_start_wy, x2=wx, y2=wy,
                                             id=_next_id("wall"))
                        walls.append(new_wall)
        ...
        # Boundary box
        pygame.draw.line(screen, (255, 255, 255), (config.BOX_LEFT, config.BOX_BOTTOM),
                         (config.BOX_RIGHT, config.BOX_BOTTOM), 3)
        pygame.draw.line(screen, (255, 255, 255), (config.BOX_LEFT, config.BOX_TOP), (config.BOX_RIGHT, config.BOX_TOP),
                         3)
        pygame.draw.line(screen, (255, 255, 255), (config.BOX_LEFT, config.BOX_TOP),
                         (config.BOX_LEFT, config.BOX_BOTTOM), 3)
        pygame.draw.line(screen, (255, 255, 255), (config.BOX_RIGHT, config.BOX_TOP),
                         (config.BOX_RIGHT, config.BOX_BOTTOM), 3)
        ...
        if state.mode == "SPRING" and state.spring_selected_obj is not None:
            ...
            if isinstance(state.spring_selected_obj, bodies.Rectangle):
                len_x = max(1, int(state.spring_selected_obj.length * coords.pixels_per_meter_x))
                len_y = max(1, int(state.spring_selected_obj.width * coords.pixels_per_meter_y))
                box_x = int(config.BOX_LEFT + (
                            state.spring_selected_obj.x - state.spring_selected_obj.length / 2) * coords.pixels_per_meter_x)
                box_y = int(config.BOX_BOTTOM - (
                            state.spring_selected_obj.y + state.spring_selected_obj.width / 2) * coords.pixels_per_meter_y)
                pygame.draw.rect(screen, (255, 255, 255), (box_x - 3, box_y - 3, len_x + 6, len_y + 6), 2)
            else:
                rp = max(1, int(state.spring_selected_obj.radius * coords.pixels_per_meter_x))
                pygame.draw.circle(screen, (255, 255, 255), (sx, sy), rp + 3, 2)
